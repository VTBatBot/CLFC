...
// Channel of the DAC to use (0 for DAC0, 1 for DAC1)
...
// Generate the waveform for a CF-FM pulse
void generate_chirp()
{ 
  const double f_max = 60e3;
  const double bw = 20e3;
  const int n = 20;

  const double fs = num_dac_samples/chirp_duration;
  const double dt = 1/fs;

  double f_sum = 0;
  const double k = exp(log(bw)/n) - 1;
  for (int i = 0; i < num_dac_samples; ++i) {
    const double ratio = (double)i/(num_dac_samples - 1);
    // Instantaneous frequency
    f_sum += f_max - pow(ratio + k, n);
    // Instantaneous phase
    const double phi = 2 * M_PI * f_sum * dt;
    // Apply a Hanning window
    const double window = 0.5 * (1 - cos(2 * M_PI * ratio));
    const double y = cos(phi) * window;
    // Move the signal from [-1, 1] to [0, 4096)
    output_waveform[i] = 4096/2 * (1 + y);
  }
}
...
void loop() {
  ...
  if (SerialUSB.available() >= 5) {
    ...
    switch (opcode) {
      ...
      // Collect data (start dynamic)
      case 0x03: {
        char response[5] = { opcode | 0x80, 0, 0, 0, 0 };
        SerialUSB.write(response, 5);

        Serial1.write('s');
        // Wait a little after triggering for the motion to begin
        delay(30);
        collect_data = true;
