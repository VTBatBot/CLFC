...
void dma_init() {
  // trigger on TCC OVF, update TCC duty, circular
   ...
   static DmacDescriptor descriptor2 __attribute__((aligned(16)));
   static DmacDescriptor wrb __attribute__((aligned(16)));
   static uint32_t chnl0 = 0;  // DMA channel

  DMAC->BASEADDR.reg = (uint32_t)&descriptor;   //OK
  ...
  descriptor.BTCNT.reg           = HWORDS;   //Start with 20 transactions
  descriptor.SRCADDR.reg         = (uint32_t)&data + 2*HWORDS;   
  descriptor.DSTADDR.reg         = (uint32_t)&DAC->DATA[0].reg;

  descriptor2.DESCADDR.reg = (uint32_t) &descriptor;
  ...
  descriptor2.BTCNT.reg           = HWORDS;   //Start with 20 transactions
  descriptor2.SRCADDR.reg         = (uint32_t)&data + 2*HWORDS;   
